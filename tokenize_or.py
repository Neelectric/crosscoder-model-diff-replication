...
def sample_half_toks(or_raw_dataset, max_num_tokens):
    raw_instances = []
    formatted_conversations = []
    tokenized_conversations = []
    num_tokens = 0
    ds_columns = or_raw_dataset.column_names
    ...
    for i in range(0, len(or_raw_dataset), 100):
        instances = formatted_ds[i:min(i+100, len(or_raw_dataset))]["formatted_chat"]
...
@torch.no_grad
def main():
    max_num_tokens = 400_000_000
    print(f"Setting max_num_tokens to {max_num_tokens}")

    or_raw_dataset = load_dataset("open-r1/OpenR1-Math-220k", "default")["train"]
    raw_instances, formatted_conversations, tokenized_conversations, num_tokens = sample_half_toks(or_raw_dataset, max_num_tokens//2)
    ...
    dataset.save_to_disk("/home/user/repos/R1-crosscoder/data/openr1-math-220k-qwen-2.5-math-1.5b")
    dataset.push_to_hub("Neelectric/openr1-math-220k-qwen-2.5-math-1.5b")
